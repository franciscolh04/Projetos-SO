...
void sort_seats(size_t num_seats, size_t* xs, size_t* ys) {
    // Algoritmo de ordenação - Bubble Sort (poderia ser substituído por um algoritmo mais eficiente)
...
int ems_reserve(unsigned int event_id, size_t num_seats, size_t* xs, size_t* ys) {
  ...
  sort_seats(num_seats, xs, ys); // Ordena lugares antes de os reservar
  for (size_t a = 0; a < num_seats; a++) {
    printf("Reserve vai dar lock neste lugar  %lu;%lu\n",xs[a],ys[a]);
  ...
  if (i < num_seats) {
    ...
    for (size_t k = 0; k < num_seats; k++) {
      printf("Reserve vai dar unlock neste lugar  %lu;%lu\n",xs[k],ys[k]);
...
int ems_show(int fd, unsigned int event_id) {
  ...
  for (size_t i = 1; i <= event->rows; i++) {
    for (size_t j = 1; j <= event->cols; j++) {
      //printf("show vai dar lock neste lugar  %lu;%lu\n",i,j);
...
void* execute_commands(void *args) {
  ...
  while (flag == 1) {
    ...
    printf("estou há espera de lock\n");
